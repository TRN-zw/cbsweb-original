(function (module) {
    mifosX.controllers = _.extend(module, {
        ViewLoanDetailsController: function (scope, routeParams, resourceFactory, paginatorService, location,  $uibModal, route, http, dateFilter, $sce, $rootScope, API_VERSION) {
            scope.loandocuments = [];
            scope.report = false;
            ...
            scope.clickEvent = function (eventName, accountId) {
              console.log(eventName + " " + accountId);
                eventName = eventName || "";
                switch (eventName) {
                    case "addloancharge":
            ...
            resourceFactory.LoanAccountResource.getLoanAccountDetails({loanId: routeParams.id, associations: 'all',exclude: 'guarantors,futureSchedule'}, function (data) {
                ...
                if(data.inArrears){
                  ...
                  var arrearDate = new Date(data.summary.overdueSinceDate);
                  var timeDiff = Math.abs(today.getTime() - arrearDate.getTime());
                  scope.date.overdueDate = Math.floor(timeDiff / (1000 * 3600 * 24));
                   //var future = new moment();
                   // var start =  moment(data.summary.overdueSinceDate);
                   // scope.date.overdueDate = moment().diff(moment(data.summary.overdueSinceDate), 'days');
                  // scope.date.overdueDate = new Date(loandetails.summary.overdueSinceDate);
                }
