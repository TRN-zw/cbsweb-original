(function (module) {
    mifosX.controllers = _.extend(module, {
        MakeDataTableEntryController: function (scope, location, routeParams, resourceFactory, dateFilter) {
            ...
            scope.formDat = {};
            scope.tf = "HH:mm";
            resourceFactory.DataTablesResource.getTableDetails({ datatablename: scope.tableName,  genericResultSet: 'true' }, function (data) {
                console.info(data);
                ...
                for (var i in data.columnHeaderData) {
                    if (data.columnHeaderData[i].columnDisplayType == 'DATETIME') {
                        scope.formDat[data.columnHeaderData[i].columnName] = {};
                    }
                }
                scope.columnHeaders = data.columnHeaderData;
