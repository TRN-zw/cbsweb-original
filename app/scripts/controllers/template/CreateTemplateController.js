        ...
        CreateTemplateController: function (scope, resourceFactory, location, $rootScope) {
            ...
            scope.loanKeys = function () {
                ...
                scope.loanTemplateKeys = ["{{loan.accountNo}}", "{{loan.status.value}}", "{{loan.loanProductId}}",
                    "{{loan.loanProductName}}", "{{loan.loanProductDescription}}", "{{loan.clientName}}", "{{loan.loanOfficerName}}",
                        "{{loan.principal}}", "{{loan.numberOfRepayments}}", "{{loan.interestRatePerPeriod}}", "{{loan.interestType.value}}",
                            "{{loan.repaymentFrequencyType.value}}", "{{loan.repaymentEvery}}"];
                scope.repaymentTemplateKeys = ["<div ng-include='repaymentschedule.html'></div>","{{loan.repaymentSchedule.loanTermInDays}}", "{{loan.repaymentSchedule.totalPrincipalDisbursed}}",
